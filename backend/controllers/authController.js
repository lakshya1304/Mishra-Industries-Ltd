...
export const loginUser = asyncHandler(async (req, res) => {
  ...
  success(res, "Successfully logged in", {
    _id: user._id,
    fullName: user.fullName,
    email: user.email,
    phone: user.phone,
...
export const forgotPassword = asyncHandler(async (req, res) => {
  ...
  user.otp = hashOTP;
  user.otpExp = expiry;
  let otpSent = await user.save();

  let mailSent = await sendEmail(email, "Reset OTP", OTP);
  if (!otpSent || !mailSent) {
    return err(res, `Error processing OTP facility`, 500);
  }
...
export const verifyOTP = asyncHandler(async (req, res) => {
  ...
  if (user.otp != otp) {
    return err(res, "Invalid OTP", 400);
  }
  ...
  if (!updated) {
    return err(res, "OTP validation failed", 500);
  }
